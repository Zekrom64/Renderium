...
public class StructStack<@NonNull S> implements ISafeCloseable {
	...
	protected final IStructAccessor<S> accessor;
	// The buffer storing stack values
	protected final ByteBuffer stackBuffer;
	...
	public StructStack(int size, IStructAccessor<S> accessor, S cacheval) {
		this.accessor = accessor;
		stackBuffer = MemoryUtil.memAlloc(accessor.getSizeOf() * size);
		maxSize = size;
		stackPointer = size;
		topCache = cacheval;
	}
